"""
world_e_wbv_surrogate_error_full_standalone.py

WORLD E: WBV SURROGATE MIS-SPECIFICATION WORLD

We inject systematic + random error ONLY into WBV, simulating
a mis-calibrated viscosity surrogate model or measurement.

Levels:
    bias = 0.00, 0.10, 0.20, 0.30
    (random noise fixed at rel_std = 0.05)

Output:
    preprocessed_outputs/worldE_wbv_error_results.csv
"""
...
OUTPUT_CSV = OUTPUT_DIR / "worldE_wbv_error_results.csv"
...
WBV_BIAS_LEVELS = [0.00, 0.10, 0.20, 0.30]
WBV_REL_STD = 0.05
...
def apply_wbv_error_inplace(df: pd.DataFrame, bias: float, rel_std: float, seed: int) -> None:
    if "WBV" not in df.columns:
        raise KeyError("Column 'WBV' not found in dataframe for World E.")

    if bias == 0.0 and rel_std <= 0.0:
        return

    vals = df["WBV"].to_numpy(float)
    rng = np.random.default_rng(seed)
    noise = rng.normal(1.0, rel_std, size=len(vals))
    wbv_noisy = vals * (1.0 + bias) * noise
    wbv_noisy = np.clip(wbv_noisy, a_min=0.0, a_max=None)
    df["WBV"] = wbv_noisy
...
def main() -> None:
    ...
    for bias in WBV_BIAS_LEVELS:
        print(f"\n====== WORLD E — wbv_bias = {bias:.2f} ======")

        df_train = df.loc[train_idx].copy()
        df_test = df.loc[test_idx].copy()

        # apply surrogate error on BOTH train and test
        apply_wbv_error_inplace(df_train, bias, WBV_REL_STD, seed=RANDOM_STATE + 100 + int(bias * 1000))
        apply_wbv_error_inplace(df_test, bias, WBV_REL_STD, seed=RANDOM_STATE + 200 + int(bias * 1000))
        ...
        res_df["wbv_bias"] = bias
        all_results.append(res_df)

    final_df = pd.concat(all_results, ignore_index=True)
    final_df.to_csv(OUTPUT_CSV, index=False)

    print("\nDONE — World E results saved to:", OUTPUT_CSV)
